...
public class MultiSegmentSelectQueryTest
{
  ...
  private Druids.SelectQueryBuilder newBuilder(boolean cursorForNext)
  {
    return Druids.newSelectQueryBuilder()
                 ...
                 .pagingSpec(PagingSpec.newSpec(3, cursorForNext));
  }
  ...
  public void testAllGranularity()
  {
    ...
    runAllGranularityTest(
        newBuilder(true).build(),
        new int[][]{
            {3, -1, -1, -1, 3}, {4, 2, -1, -1, 3}, {-1, 4, 1, -1, 3}, {-1, -1, 4, -1, 3}, {-1, -1, 5, 2, 3},
            {-1, -1, -1, 5, 3}, {-1, -1, -1, 8, 3}, {-1, -1, -1, 11, 3}, {-1, -1, -1, 13, 2}, {-1, -1, -1, 13, 0}
        }
    );

    runAllGranularityTest(
        newBuilder(false).descending(true).build(),
        new int[][]{
    ...
    runAllGranularityTest(
        newBuilder(true).descending(true).build(),
        new int[][]{
            {0, 0, 0, -4, 3}, {0, 0, 0, -7, 3}, {0, 0, 0, -10, 3}, {0, 0, 0, -13, 3}, {0, 0, -3, -14, 3},
            {0, 0, -6, 0, 3}, {0, -4, 0, 0, 3}, {-3, -5, 0, 0, 3}, {-5, 0, 0, 0, 2}, {-5, 0, 0, 0, 0}
        }
    );
  }
  ...
  private void runAllGranularityTest(SelectQuery query, int[][] expectedOffsets)
  {
    for (int[] expected : expectedOffsets) {
      ...
      LinkedHashMap<String, Integer> merged = PagingSpec.merge(Arrays.asList(pagingIdentifiers));
  ...
  public void testDayGranularity()
  {
    runDayGranularityTest(
        newBuilder(false).granularity(QueryRunnerTestHelper.dayGran).build(),
        new int[][]{
    ...
    runDayGranularityTest(
        newBuilder(true).granularity(QueryRunnerTestHelper.dayGran).build(),
        new int[][]{
            {3, -1, -1, 3, 3, 0, 0, 3}, {4, 2, -1, 6, 1, 2, 0, 3}, {-1, 4, 1, 9, 0, 2, 1, 3},
            {-1, -1, 4, 12, 0, 0, 3, 3}, {-1, -1, 5, 13, 0, 0, 1, 1}, {-1, -1, 5, 13, 0, 0, 0, 0}
        }
    );

    runDayGranularityTest(
        newBuilder(false).granularity(QueryRunnerTestHelper.dayGran).descending(true).build(),
        new int[][]{
    ...
    runDayGranularityTest(
        newBuilder(true).granularity(QueryRunnerTestHelper.dayGran).descending(true).build(),
        new int[][]{
            {0, 0, -4, -4, 0, 0, 3, 3}, {0, -2, -6, -7, 0, 1, 2, 3}, {0, -5, 0, -10, 0, 3, 0, 3},
            {-4, 0, 0, -13, 3, 0, 0, 3}, {-5, 0, 0, -14, 1, 0, 0, 1}, {-5, 0, 0, -14, 0, 0, 0, 0}
        }
    );
  }
  ...
  private void runDayGranularityTest(SelectQuery query, int[][] expectedOffsets)
  {
    for (int[] expected : expectedOffsets) {
      ...
      LinkedHashMap<String, Integer> merged =
          PagingSpec.merge(Arrays.asList(pagingIdentifiers0, pagingIdentifiers1));
  ...
  private PagingSpec toNextCursor(LinkedHashMap<String, Integer> merged, SelectQuery query, int threshold)
  {
    if (!query.isCursorForNext()) {
      merged = PagingSpec.next(merged, query.isDescending());
    }
    return new PagingSpec(merged, threshold, query.isCursorForNext());
  }
}
